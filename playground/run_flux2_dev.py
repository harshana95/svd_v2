#!/usr/bin/env python3
"""
Run FLUX.2-dev with diffusers (text-to-image, or text + single reference image).

Examples:
  python playground/run_flux2_dev.py --prompt "a photo of a corgi in space" --out out.png
  python playground/run_flux2_dev.py --prompt "same scene at golden hour" --image ref.png --out out.png
"""
...
def _load_flux2_dev_pipeline(model_id: str, torch_dtype: torch.dtype) -> Any:
    """
    Try a few imports so this script works across diffusers versions.
    """
    # Preferred for FLUX.2 in newer diffusers.
    try:
    ...
    # Some versions may still expose only FluxPipeline.
    try:
    ...
    # Fallback import paths used by some builds/forks.
    try:
    ...
    try:
        ...
        raise RuntimeError(
            "Could not import Flux2Pipeline/FluxPipeline for FLUX.2-dev from diffusers. "
            "Your diffusers version may not include this pipeline. "
            "Try upgrading diffusers or verify the pipeline class name."
        ) from e

def get_vjepa2_model(hub_entry: str, dtype: torch.dtype, pretrained: bool = False, vjepa_checkpoint_path: str = None) -> Any:
...
def main() -> None:
    p = argparse.ArgumentParser(description="Run FLUX.2-dev with diffusers.")
    p.add_argument(
        "--model",
        default=os.environ.get("FLUX2_DEV_MODEL", "black-forest-labs/FLUX.2-dev"),
        help="HuggingFace model id or local path. Env: FLUX2_DEV_MODEL",
    )
    ...
    p.add_argument(
        "--image",
        default=None,
        help="Path to one reference image (RGB). Passed as `image` to Flux2Pipeline for conditioning.",
    )
    p.add_argument(
        "--height",
        type=int,
    ...
    p.add_argument("--out", default="flux2_dev_out.png", help="Output path (png recommended).")
    ...
    if args.image:
        ...
        if args.height is None and args.width is None:
            # FLUX.2 memory grows quickly with resolution. Keep default image-conditioned
            # runs in a safer range unless the user explicitly sets height/width.
            max_side = 1024
            src_w, src_h = ref_pil.size
            if max(src_w, src_h) > max_side:
                scale = max_side / float(max(src_w, src_h))
                auto_w = _round_down_to_multiple(int(src_w * scale))
                auto_h = _round_down_to_multiple(int(src_h * scale))
                args.width = auto_w
                args.height = auto_h
                print(
                    f"Auto-setting size to {auto_w}x{auto_h} (source {src_w}x{src_h}) "
                    "to reduce OOM risk. Override with --width/--height."
                )
    ...
    if args.use_jepa:
        ...
        vjepa = get_vjepa2_model(
            "vjepa2_1_vit_large_384", vjepa_dtype, True, None
        ).to(device)
    ...
    pipe = _load_flux2_dev_pipeline(args.model, torch_dtype=dtype)
