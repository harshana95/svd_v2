...
if __name__ == "__main__":
    ...
    parser.add_argument('--wv', action='store', type=int, default=[472.5, 532.8, 685])
    ...
    parser.add_argument('--psf_type', default='')
    args = parser.parse_args()
    if not isinstance(args.wv, list):
        args.wv = [args.wv]
    ...
    elif args.psf_type == 'motion':
        kwargs = {'psf_type': 'motion'}
    elif args.psf_type == 'deeplens1':
        kwargs = {'psf_type': 'deeplens', 'lens_path': '/depot/chan129/users/harshana/svd_v2/lenses/cooke40_inferior.json', 'sensor_res': (1024, 1024)}
    elif args.psf_type == 'deeplens2':
        kwargs = {'psf_type': 'deeplens', 'lens_path': '/depot/chan129/users/harshana/svd_v2/lenses/thorlabs/acl12708u.json', 'sensor_res': (1024, 1024)}
    else:
        raise ValueError(f"Unknown psf type {args.psf_type}")
    ...
    generate_folder(loc)
    metadata = vars(args)
    psfsim = PSFSimulator(image_shape=args.sensor_res, psf_size=args.max_psf_size, normalize=True, wavelengths=args.wv)
    ...
    # generate PSFs
    generated_psf, psf_metadata = psfsim.generate_all_psfs(metadata['grid_size'][0], metadata['grid_size'][1], outc=1, inc=len(args.wv), **kwargs)
