...
class PipelineOutput(BaseOutput):
    ...
    refined_1: Union[List[PIL.Image.Image], np.ndarray, torch.Tensor]
    refined_2: Union[List[PIL.Image.Image], np.ndarray, torch.Tensor]
...
class DiffusionTwoImagePipeline(
    DiffusionPipeline,
    FromSingleFileMixin,
):
    def __init__(
    ...
    def __call__(
        ...
        refined_1 = torch.nn.functional.interpolate(refined_1, (ha, wa), mode='bilinear')
        image_2 = torch.nn.functional.interpolate(image_2, (hb, wb), mode='bilinear')
        refined_2 = torch.nn.functional.interpolate(refined_2, (hb, wb), mode='bilinear')
        refined_1 = refined_1.to(prompt_embeds.dtype)
        refined_2 = refined_2.to(prompt_embeds.dtype)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if (output_intermediate_steps and i % (len(timesteps) // n_output_intermediate_steps) == 0) \
        ...
        return PipelineOutput(images=step_outputs[-1], refined_1=refined_1, refined_2=refined_2,
                              step_outputs=step_outputs, timesteps=timesteps)
