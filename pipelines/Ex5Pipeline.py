...
class Ex5Pipeline(
    ...
    def __call__(
        ...
        # 4. Prepare timesteps
        timesteps, num_inference_steps = retrieve_timesteps(self.scheduler, len(timesteps), device, timesteps=timesteps)
        if len(timesteps) < n_output_intermediate_steps:
            n_output_intermediate_steps = len(timesteps)
