...
class NAFNet(nn.Module):
    ...
    def __init__(self, in_channel=3, out_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[]):
        ...
        self.middle_blks = \
            TimestepEmbedSequential(
                *[NAFBlock(chan, time_embed_dim) for _ in range(middle_blk_num)]
            )
    ...
    def forward(self, inp, timesteps=None):
        ...
        x = self.intro(inp)
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels)).type(inp.dtype)
...
class NAFNetEmb_arch(PreTrainedModel):
    config_class = NAFNetEmb_config
