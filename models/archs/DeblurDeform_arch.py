...
class DiTTransformer2D_arch(ModelMixin, ConfigMixin):
    r"""
    ...
    def forward(
        ...
        """
        if timestep is None:
            timestep = 999*torch.ones(hidden_states.shape[0], dtype=torch.long, device=hidden_states.device)
        if class_labels is None:
            class_labels = torch.zeros(hidden_states.shape[0], dtype=torch.long, device=hidden_states.device)
        # 1. Input
