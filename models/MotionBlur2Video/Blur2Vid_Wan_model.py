"""
Blur2Vid model using Wan2.1 I2V (image-to-video)
Uses Wan2.1's flow-matching formulation with blur image as conditioning via CLIP image encoder.
"""
import cv2
import numpy as np
import torch
...
from models.MotionBlur2Video.helpers import apply_lora_to_model
...
class Blur2Vid_Wan_model(BaseModel):
    """Generate video from motion blurred image using Wan2.1 I2V (flow matching + CLIP image conditioning)."""

    def __init__(self, opt, logger):
        super(Blur2Vid_Wan_model, self).__init__(opt, logger)
        self.load_handle.remove()
        self.load_handle = self.accelerator.register_load_state_pre_hook(load_model_hook)
        
        args = self.opt["args"]
        self.args = args
        model_id = getattr(args, "pretrained_model_name_or_path", "Wan-AI/Wan2.1-I2V-14B-480P-Diffusers")

        # Wan2.1 I2V components
        self.tokenizer = AutoTokenizer.from_pretrained(model_id, subfolder="tokenizer", revision=getattr(args, "revision", None) or "main")
        self.text_encoder = UMT5EncoderModel.from_pretrained(
            model_id, subfolder="text_encoder", revision=getattr(args, "revision", None) or "main"
        )
        self.image_processor = CLIPImageProcessor.from_pretrained(
            model_id, subfolder="image_processor", revision=getattr(args, "revision", None) or "main"
        )
        self.image_encoder = CLIPVisionModel.from_pretrained(
            model_id, subfolder="image_encoder", revision=getattr(args, "revision", None) or "main"
        )
        ...
        self.vae = AutoencoderKLWan.from_pretrained(
            model_id, subfolder="vae", torch_dtype=torch.float32, revision=getattr(args, "revision", None) or "main"
        )
        self.scheduler = FlowMatchEulerDiscreteScheduler.from_pretrained(
            model_id, subfolder="scheduler", revision=getattr(args, "revision", None) or "main"
        )
        ...
        self.latents_std = 1.0 / torch.tensor(self.vae.config.latents_std).view(1, z_dim, 1, 1, 1)
        ...
        if self.accelerator.state.deepspeed_plugin:
            if "bf16" in getattr(self.accelerator.state.deepspeed_plugin.deepspeed_config or {}, "bf16", {}):
                weight_dtype = torch.bfloat16
            elif "fp16" in getattr(self.accelerator.state.deepspeed_plugin.deepspeed_config or {}, "fp16", {}):
    ...
    def optimize_parameters(self):
        ...
        with torch.no_grad():
            ...
            blur_img = self.sample["blur_img"].to(memory_format=torch.contiguous_format).float()
            prompts = self.sample["caption"]
            batch_size = len(prompts)
            guidance_mask = torch.rand(batch_size, device=self.accelerator.device) >= 0.2
            per_sample_prompts = [prompts[i] if guidance_mask[i] else "" for i in range(batch_size)]
            ...
            latent_condition = encode_video_wan(
                video_condition, self.accelerator, self.vae, self.latents_mean, self.latents_std
            )
            ...
            if mask_channels > 0:
                ...
                mask_lat_size[:, :, 1:] = 0
                mask_lat_size = mask_lat_size.repeat(1, mask_channels, 1, 1, 1) # mask_channels = 4
        ...
        noise_16 = torch.randn_like(model_input_16, device=model_input_16.device, dtype=model_input_16.dtype)
        batch_size, num_channels, num_frames, height, width = model_input_16.shape

        timesteps = torch.rand(batch_size, device=model_input_16.device, dtype=model_input_16.dtype)  # [B]
        ...
        velocity_target = noise_16 - model_input_16  # [B, z_dim, T, H, W]; transformer predicts this
        ...
        timestep_int = (timesteps * (num_train_timesteps - 1)).long().clamp(0, num_train_timesteps - 1)
    ...
    def validate_step(self, _batch, idx):
        ...
        num_frames_wan = ((num_frames - 1) // self.vae_scale_factor_temporal) * self.vae_scale_factor_temporal + 1
        num_frames_wan = max(num_frames_wan, 1)

        generator = torch.Generator(device=self.accelerator.device).manual_seed(self.args.seed) if getattr(self.args, "seed", None) else None
        out = self.pipeline(
            image=frame,
