...
from pipelines.DiffusionPipeline import DiffusionTwoImagePipeline
...
class Diffusion_TwoInput_model(TwoDatasetBasemodel):
    ...
    def __init__(self, opt, logger):
        super(Diffusion_TwoInput_model, self).__init__(opt, logger)
    ...
    def optimize_parameters(self):
        ...
        refined_2 = einops.repeat(refined_2, 'b 1 h w -> b c h w', c=3)
    ...
    def validate_step(self, batch, idx,lq_key,gt_key,rf_key):
        ...
        if self.opt.val.patched:
            ...
            lq2 = einops.repeat(lq2, 'b 1 h w -> b c h w', c=3)
            refined_1 = self.sample[rf_key+"_1"]
            refined_2 = self.sample[rf_key+"_2"]
            refined_2 = einops.repeat(refined_2, 'b 1 h w -> b c h w', c=3)
