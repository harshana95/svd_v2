...
class DiffusionLKPNSTN_TwoInput_model(DiffusionLKPN_TwoInput_model):

    def __init__(self, opt, logger):
        super(DiffusionLKPNSTN_TwoInput_model, self).__init__(opt, logger)
    ...
    def validation(self):
        ...
        self.pipeline = DiffusionTwoImageLKPNSTNPipeline(
            self.vae,
            self.unet,
            self.lkpn_1,
            self.lkpn_2,
    ...
    def validate_step(self, batch, idx,lq_key,gt_key):
        ...
        if self.opt.val.patched:
            ...
            output = self.pipeline(
                ...
                rescale_image_a=1, 
                rescale_image_b=1,
                num_inference_steps=self.opt.val.get('num_inference_steps', 50),
                output_intermediate_steps=True,
                n_output_intermediate_steps=10,
            )
            out = output.images
        ...
        for i in range(len(gt)):
            ...
            for step in predeblur2:
                image4.append(step[i].cpu().numpy())
