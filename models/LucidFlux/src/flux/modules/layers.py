# Ported from BFL's Flux (Apache 2.0) — transformer building blocks.
...
class Modulation(nn.Module):
    ...
    def forward(
        ...
        out = (
            self.lin(nn.functional.silu(vec))[:, None, :]
...
class DoubleStreamBlockProcessor:
    def __call__(
        self, attn, img, txt, vec, pe, **kwargs
    ):
...
class SingleStreamBlockProcessor:
    def __call__(
        ...
        qkv, mlp = torch.split(
            attn.linear1(x_mod),
            [3 * attn.hidden_size, attn.mlp_hidden_dim],
            dim=-1,
        )

        q, k, v = rearrange(
            qkv, "B L (K H D) -> K B H L D", K=3, H=attn.num_heads
        )
        q, k = attn.norm(q, k, v)
